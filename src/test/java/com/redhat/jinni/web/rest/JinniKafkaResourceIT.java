...
import com.redhat.jinni.config.KafkaProperties;
...
class JinniKafkaResourceIT {
    ...
    void setup() {
        ...
        JinniKafkaResource kafkaResource = new JinniKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jinni-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jinni-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
