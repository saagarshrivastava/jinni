import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { Authority } from 'app/shared/constants/authority.constants';
import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { IOfferType, OfferType } from 'app/shared/model/offer-type.model';
import { OfferTypeService } from './offer-type.service';
import { OfferTypeComponent } from './offer-type.component';
import { OfferTypeDetailComponent } from './offer-type-detail.component';
import { OfferTypeUpdateComponent } from './offer-type-update.component';

@Injectable({ providedIn: 'root' })
export class OfferTypeResolve implements Resolve<IOfferType> {
  constructor(private service: OfferTypeService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IOfferType> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((offerType: HttpResponse<OfferType>) => {
          if (offerType.body) {
            return of(offerType.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new OfferType());
  }
}

export const offerTypeRoute: Routes = [
  {
    path: '',
    component: OfferTypeComponent,
    data: {
      authorities: [Authority.USER],
      pageTitle: 'jinniApp.offerType.home.title'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: ':id/view',
    component: OfferTypeDetailComponent,
    resolve: {
      offerType: OfferTypeResolve
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'jinniApp.offerType.home.title'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: 'new',
    component: OfferTypeUpdateComponent,
    resolve: {
      offerType: OfferTypeResolve
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'jinniApp.offerType.home.title'
    },
    canActivate: [UserRouteAccessService]
  },
  {
    path: ':id/edit',
    component: OfferTypeUpdateComponent,
    resolve: {
      offerType: OfferTypeResolve
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'jinniApp.offerType.home.title'
    },
    canActivate: [UserRouteAccessService]
  }
];
